...
def save_payment_data(order_id, result_df, search_params, user_email, flow, payment_id=None, already_processed=False):
    ...
    try:
        ...
        cols_to_keep = [c for c in ["Город", "Вуз", "Факультет", "Код и специальность", "Профиль",
            ...
            "Конкурсный балл", "Шансы", "Рек. приоритет", "Стоимость обучения (Москва и СПб), тыс руб"] if c in result_df.columns]
...
- 🔹 **Квоты и БВИ** — подавать заявление можно, но вероятность того, что для общего конкурса останутся места после зачисления квотников и олимпиадников, небольшая. Это станет известно 3 августа
...
- ⬜ **Нет оценки — не указан балл за ДВИ** — специальность требует дополнительное вступительное испытание в вузе, но вы не указали свой балл за него. Подать заявление можно — балл вы получите уже в вузе после прохождения испытания. Оценить шансы заранее без этого балла не получится.
...
- Три листа: Важная информация, Результаты, Запрос
...
**Стоимость: 2 490 руб.**
        ...
        user_email_input = st.text_input(
            "📧 Email для получения таблицы *",
            placeholder="example@mail.ru",
            help="На этот адрес придёт таблица сразу после оплаты",
...
Запишитесь на персональную консультацию со скидкой 500 руб. по промокоду **VUZLINE500**
    ...
    if flow == 2 and len(result_backup) > 0:
        ...
        st.caption("Эти вузы не входят в выбранные вами профессиональные области, но ваши предметы и баллы для них подходят и шансы хорошие. Стоит рассмотреть как запасной вариант на случай если вам важен именно бюджет без риска.")
        if not paid:
            preview_cols = ["Город", "Вуз", "Факультет", "Код и специальность", "Профиль"]
...
Если вы поступаете по квоте, как победитель олимпиады (БВИ) или на платное обучение — этот сервис вам не подойдёт. С такими кейсами приходите на [персональную консультацию](https://vuzline.ru/consultation).
""")
...
if flow == "🔍 Подобрать варианты по моим ЕГЭ":
    st.caption("Подходит если ещё не определились куда хотите — покажем все вузы и специальности которые подходят под ваши баллы и предметы.")
else:
    st.caption("Подходит если уже есть конкретные вузы или специальности — проверим шансы именно по вашему списку.")
...
if flow == "🔍 Подобрать варианты по моим ЕГЭ":
    ...
    show_dvi = st.toggle(
        "Показывать специальности с ДВИ (дополнительным вступительным испытанием)", value=False,
        help="ДВИ — это письменный экзамен или творческое испытание которое вуз проводит самостоятельно, помимо ЕГЭ. Например, МГУ и МГИМО требуют ДВИ на большинстве направлений."
    )
    if not show_dvi:
        st.caption("💡 Не видите МГУ, МГИМО и другие топ-вузы? Включите этот тумблер — большинство их программ требуют дополнительное испытание в самом вузе.")
    selected_areas = st.multiselect(
        "Профессиональные области (необязательно)",
        list(AREA_GROUPS.keys()),
        help="Выберите интересующие вас области — они будут показаны в первую очередь. Если не выбрать — покажем все подходящие специальности сразу."
    )
    if selected_areas:
        st.caption("Вузы по выбранным областям — в первом блоке. Подстраховочные варианты из других областей — ниже.")
    else:
        st.caption("Необязательно. Если выбрать — сначала покажем вузы по нужным вам областям, остальные — как подстраховка. Если не выбирать — покажем все подходящие варианты сразу.")
...
- Проверьте что введены все сданные предметы и баллы
- Если баллы невысокие — попробуйте другой город, там конкурс ниже
- Включите показ специальностей с ДВИ — это расширит список вузов
- Уберите фильтр по профессиональным областям если он выбран
                """)
            else:
...
# ─── ФЛОУ 1 ───────────────────────────────────────────────────────────────
    ...
    if st.button("🎯 Найти по моему списку", type="primary"):
        ...
        if errors:
            ...
            if len(result) == 0:
                st.warning("По выбранным параметрам ничего не найдено.")
                st.info("""
**Что можно попробовать:**
- Проверьте что введены все сданные предметы и баллы
- Добавьте другие вузы или специальности
- Попробуйте выбрать другой город — возможно, нужные специальности есть там
- Возможно, у выбранных вузов нет бюджетных мест по этим специальностям
                """)
            else:
                st.session_state["last_result"] = result.to_dict()
